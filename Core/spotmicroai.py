...
class Robot:
    ...
    def __init__(self, useFixedBase=False, useStairs=True, resetFunc=None):
        ...
        for i in range(self.numRays):
            ...
            h = 0.045
            self.rayFrom.append([rayStartLen * math.sin(math.pi * 2 * float(i) / self.numRays),
                                 rayStartLen * math.cos(math.pi * 2 * float(i) / self.numRays), h])
            self.rayTo.append([rayLen * math.sin(math.pi * 2 * float(i) / self.numRays),
    ...
    def addInfoText(self, bodyPos, bodyEuler, linearVel, angularVel):
        ...
        text3 = "Vl: {:.1f} / {:.1f} / {:.1f} Va: {:.1f} / {:.1f} / {:.1f}".format(linearVel[0], linearVel[1],
                                                                                   linearVel[2],
                                                                                   angularVel[0], angularVel[1],
                                                                                   angularVel[2])
        ...
        p.addUserDebugLine([-0.3, 0, 0], [0.3, 0, 0], [0, 1, 0], parentObjectUniqueId=self.quadruped,
                           parentLinkIndex=1),
        p.addUserDebugLine([0, -0.2, 0], [0, 0.2, 0], [0, 1, 0], parentObjectUniqueId=self.quadruped,
                           parentLinkIndex=1),
    ...
    def step(self):
        ...
        if (nowLidarTime - self.lastLidarTime > .2):
            numThreads = 0
            results = p.rayTestBatch(self.rayFrom, self.rayTo, numThreads, parentObjectUniqueId=quadruped,
                                     parentLinkIndex=0)
            for i in range(self.numRays):
                ...
                if (hitFraction == 1.):
                    if self.debugLidar:
                        p.addUserDebugLine(self.rayFrom[i], self.rayTo[i], self.rayMissColor,
                                           replaceItemUniqueId=self.rayIds[i], parentObjectUniqueId=quadruped,
                                           parentLinkIndex=0)
                else:
                    ...
                    if self.debugLidar:
                        p.addUserDebugLine(self.rayFrom[i], localHitTo, self.rayHitColor,
                                           replaceItemUniqueId=self.rayIds[i], parentObjectUniqueId=quadruped,
                                           parentLinkIndex=0)
